...
- TurtleBot3 Waffle Pi를 활용한 공정 간 물류 이송
...
### Team Leader / RoboDK Digital Twin
...
- 실제 공정 흐름을 기준으로 시뮬레이션 동작을 반복 검증
...
| 구분 | 기술 |
|---|---|
| Digital Twin | RoboDK, RoboDK Python API |
| Programming | Python, C++, JavaScript, TypeScript |
| Robotics | ROS2, Dobot Magician, TurtleBot3 Waffle Pi |
| Embedded | Raspberry Pi, Conveyor Control |
| AI / Vision | YOLOv5, OpenCV, Intel RealSense |
| Communication | Modbus TCP, WebSocket, TCP/JSON |
| Web | Vue 3, Vite, Chart.js, HTML, CSS |
| Data / Test | SQLite, Python unittest |
...
├─ Modbus/                 # Modbus register, mock server 및 ROS2 bridge 초안
├─ RoboDK/
│  ├─ assets/
│  │  ├─ AGV/             # AGV 모델
│  │  ├─ Convey/          # 컨베이어 모델
│  │  ├─ Object/          # Wafer, DIE, Tray 및 공정 오브젝트
│  │  ├─ Robot/           # 로봇 모델
│  │  └─ Tool/            # 로봇 툴 모델
│  ├─ scripts/
│  │  ├─ ImportCode.py    # RoboDK 스테이션 구성 및 초기화
│  │  └─ Wafer_move.py    # 웨이퍼 이송 및 후공정 시뮬레이션
│  └─ stations/
│     └─ Penetrate_PJT.rdk
├─ WebSocket/              # Vue/TypeScript 관제 대시보드와 Gateway
├─ main_ws/                # ROS2 Workspace
├─ web/dashboard/          # 정적 웹 대시보드 MVP
├─ websocket/              # Python WebSocket Gateway MVP
├─ yolo_v5/                # 웨이퍼 탐지 모델 관련 결과 및 설정
├─ datasets/               # 학습 및 검증 데이터 관련 파일
├─ record_data/            # 프로젝트 기록 데이터
├─ images/                 # README 실행 화면
└─ README.md
```
